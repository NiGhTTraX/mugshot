  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (delta > maxDelta) {
        ...
        const val = grayPixel(img1, pos, alpha);
...
function antialiased(img: Buffer, x1: number, y1: number, width: number, height: number, img2: Buffer) {
...
function hasManySiblings(img: Buffer, x1: number, y1: number, width: number, height: number) {
  const x0 = Math.max(x1 - 1, 0);
  const y0 = Math.max(y1 - 1, 0);
  const x2 = Math.min(x1 + 1, width - 1);
  const y2 = Math.min(y1 + 1, height - 1);
