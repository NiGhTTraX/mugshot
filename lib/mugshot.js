 ...
 * @const
 */
var DEFAULT_PNGPROCESSOR = require('./adaptors/png-crop.js');
...
Mugshot.prototype.test = function(captureItem, callback) {
  ...
  if (callback === undefined || typeof callback !== 'function') {
    throw new Error('You have not provided a correct callback');
  }
  ...
  fs.mkdir(this._options.rootDirectory, function(error) {
    ...
    browser.takeScreenshot(function(error, screenshot) {
      ...
      processScreenshot(captureItem, browser, options, screenshot);
    });
  });
};
 ...
 *
 * @param captureItem - Same as in test method
 * @param {Browser} browser - A browser adaptor instance
 * @param {Object} options - Same as in test method
 * @param {Buffer} screenshot - A binary representation of the whole web page
 */
function processScreenshot(captureItem, browser, options, screenshot) {
  var PNGProcessor = options.PNGProcessor;

  if (captureItem.selector === undefined) {
    _test(captureItem.name, options, screenshot);
  } else {
    browser.getBoundingClientRect(captureItem.selector, function(error, rect) {
      ...
      PNGProcessor.crop(screenshot, rect, function(error, croppedImage) {
        ...
        _test(captureItem.name, options, croppedImage);
      });
    });
  }
}
 ...
 */
function _test(baseName, options, screenshot) {
